...
struct NewRemoteCode {
	...
	unsigned short dimLevel;				// Dim level [0..15]. Will be available if switchType is dim, on_with_dim or off_with_dim.
};
...
class NewRemoteReceiver {
	public:
		...
		*/
//		static void init(int8_t interrupt, unsigned short minRepeats, NewRemoteReceiverCallBack callback);
		NewRemoteReceiver(int8_t interrupt, unsigned short minRepeats, NewRemoteReceiverCallBack callback);
